...
import { generateBookPlan, generateColoringPage, generateCoverImage, getClosestAspectRatio, analyzeCoverTemplate } from './services/gemini';
import { generatePDF, generateSingleImagePDF, generateCoverPDF } from './services/pdfGenerator';
...
const INITIAL_STATE: GenerationState = {
  ...
  topic: '',
  targetAge: '3-5',
  dimensions: { width: 5.5, height: 8.5, unit: 'in' },
  metadata: null,
  pages: [],
...
export default function App() {
  ...
  const handleError = (err: any) => {
    ...
    let message = (err.message || JSON.stringify(err)).toLowerCase();
    let isQuota = message.includes("quota") || message.includes("429") || message.includes("resource_exhausted");
    setError({ 
      message: isQuota ? "Quota Restricted: The engine is resting. Please wait 15s before next request." : message, 
      isQuota 
    });
    setLoading(false);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.includes('image')) {
      ...
      reader.onload = async (loadEvent) => {
        ...
        setState(prev => ({ ...prev, coverSource: 'uploaded', coverImage: base64, uploadedCoverFile: file }));
        
        setLoadingCover(true);
        try {
            const analyzed = await analyzeCoverTemplate(base64);
            if (analyzed.width && analyzed.height) {
                setState(prev => ({ 
                    ...prev, 
                    dimensions: { ...prev.dimensions, width: analyzed.width, height: analyzed.height } 
                }));
            }
      ...
      reader.readAsDataURL(file);
        ...
        setState(prev => ({ ...prev, coverSource: 'uploaded', uploadedCoverFile: file }));
    }
  };

  const generateCover = async (side: 'front' | 'back') => {
    side === 'front' ? setLoadingCover(true) : setLoadingBackCover(true);
    setError(null);
    try {
      const aspectRatio = getClosestAspectRatio(state.dimensions.width, state.dimensions.height);
      const referenceImage = (state.coverSource === 'uploaded' && state.coverImage) ? state.coverImage : undefined;
      const result = await generateCoverImage(
          state.topic, 
          state.metadata?.title || state.topic, 
          state.metadata?.subtitle || "", 
          state.metadata?.authorName || "KDP Artist", 
          aspectRatio, 
          side === 'back',
          referenceImage
      );
      setState(prev => ({ 
        ...prev, 
        [side === 'front' ? 'coverImage' : 'backCoverImage']: result,
  ...
  const handleGeneratePlan = async () => {
    ...
    setError(null);
    try {
      const plan = await generateBookPlan(state.topic, state.targetAge);
      setState(prev => ({ 
          ...prev, 
          step: 'planning', 
          metadata: plan.metadata, 
          pages: plan.pages.map((p, i) => ({ id: `p-${i}-${Date.now()}`, ...p, status: 'pending' })) 
      }));
    } catch (err) { handleError(err); }
    setLoading(false);
  };
  ...
  const generateSinglePage = async (pageId: string) => {
    ...
    setState(prev => ({ ...prev, pages: prev.pages.map(p => p.id === pageId ? { ...p, status: 'generating' } : p) }));
    
    try {
      const aspectRatio = getClosestAspectRatio(state.dimensions.width, state.dimensions.height);
      const img = await generateColoringPage(page.prompt, aspectRatio);
      setState(prev => ({ ...prev, pages: prev.pages.map(p => p.id === pageId ? { ...p, status: 'completed', imageUrl: img } : p) }));
      return true;
    } catch (err) {
  ...
  const handleGenerateAllInterior = async () => {
  ...
  const navigate = (view: ViewType) => {
    setError(null);
    setState(prev => (view === 'home' ? INITIAL_STATE : { ...prev, view }));
  };
        ...
        <div className="max-w-[1700px] mx-auto px-8 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             <nav className="hidden lg:flex items-center gap-10 mr-8">
                <button onClick={() => navigate('home')} className={`text-[10px] font-black uppercase tracking-widest transition-colors ${state.view === 'home' && state.step === 'input' ? 'text-brand-400 underline underline-offset-8 decoration-2' : 'text-slate-400 hover:text-white'}`}>Home</button>
            ...
            <>
              {state.step === 'input' && (
                <div className="max-w-4xl mx-auto mt-16 text-center animate-in fade-in duration-1000">
                  ...
                  <p className="text-xl text-slate-500 dark:text-slate-400 mb-14 font-medium max-w-2xl mx-auto leading-relaxed">Professional cover detailed design & interior engine for Amazon publishers. Analyzes structure & dimensions for perfect compliance.</p>
                  
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-stretch">
                    <div className="bg-white dark:bg-slate-900 p-12 rounded-[4rem] shadow-3xl border border-slate-100 dark:border-slate-800 text-left relative overflow-hidden flex flex-col h-full">
                      <div className="mb-6">
                          <label className="block text-[11px] font-black text-slate-400 mb-4 uppercase tracking-[0.4em]">Target Book Topic (High SEO)</label>
                          <input 
                              type="text" 
                              className="w-full px-8 py-6 text-xl font-black border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white rounded-3xl outline-none focus:border-brand-500 transition-all shadow-inner" 
                              placeholder="e.g., Magic Unicorns..." 
                              value={state.topic} 
                      ...
                      <div className="mb-6">
                          <label className="block text-[11px] font-black text-slate-400 mb-4 uppercase tracking-[0.4em] flex items-center gap-2">
                            <Baby className="h-4 w-4" /> Enter Target Age Group
                          </label>
                          <input 
                              type="text" 
                              className="w-full px-8 py-6 text-xl font-black border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white rounded-3xl outline-none focus:border-brand-500 transition-all shadow-inner" 
                              placeholder="e.g., 3-5, 6-8, Toddlers..." 
                              value={state.targetAge} 
                              onChange={(e) => setState({ ...state, targetAge: e.target.value })} 
                          />
                      </div>

                      <div className="mb-10 p-8 bg-slate-50 dark:bg-slate-950 rounded-[2.5rem] border-2 border-slate-100 dark:border-slate-800 shadow-inner">
                          <div className="flex items-center justify-between mb-6">
                              <div className="flex items-center gap-3">
                                <Ruler className="h-5 w-5 text-brand-600" />
                                <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">KDP Specifications</span>
                              </div>
                              <button onClick={() => setState({...state, dimensions: {width: 5.5, height: 8.5, unit: 'in'}})} className="text-[9px] font-black bg-brand-50 text-brand-600 px-3 py-1 rounded-full uppercase hover:bg-brand-100 transition-colors">5.5" x 8.5" Preset</button>
                          </div>
                          <div className="grid grid-cols-2 gap-8 mb-6">
                              <div>
                                  <label className="block text-[9px] font-black text-slate-500 mb-2 uppercase tracking-widest">Width (in)</label>
                                  <input 
                                      type="number" 
                                      step="0.001"
                                      className="w-full px-6 py-4 font-black border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl outline-none focus:border-brand-500 transition-all"
                                      value={state.dimensions.width}
                                      onChange={(e) => setState({...state, dimensions: {...state.dimensions, width: parseFloat(e.target.value)}})}
                                  />
                              </div>
                              <div>
                                  <label className="block text-[9px] font-black text-slate-500 mb-2 uppercase tracking-widest">Height (in)</label>
                                  <input 
                                      type="number" 
                                      step="0.001"
                                      className="w-full px-6 py-4 font-black border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl outline-none focus:border-brand-500 transition-all"
                          ...
                          <div className="p-4 bg-white/50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
                             <p className="text-[9px] font-bold text-slate-400 leading-tight">Obligations provided: Livre relié 5.5" x 8.5". Global: 13.102" x 9.917". Spine: 0.527". Interior: 150 Pages B&W.</p>
                          </div>
                      </div>

                      <div className="flex flex-col gap-4 mt-auto">
                        <button onClick={handleGeneratePlan} disabled={loading || !state.topic.trim()} className="w-full bg-brand-950 dark:bg-brand-600 hover:bg-brand-900 text-white font-black py-7 rounded-[2.5rem] shadow-2xl flex items-center justify-center gap-5 text-xl uppercase tracking-widest disabled:opacity-50 transition-all active:scale-[0.98]">
                          {loading ? <Loader2 className="animate-spin h-8 w-8" /> : (
                            <>Draft Book Plan <ChevronRight className="h-6 w-6" /></>
                          )}
                        </button>
                      </div>
                    </div>

                    <div className="bg-white dark:bg-slate-900 p-12 rounded-[4rem] shadow-3xl border border-slate-100 dark:border-slate-800 text-left relative overflow-hidden flex flex-col justify-center items-center h-full group">
                         <div className="flex flex-col items-center gap-8">
                            <div className="p-12 bg-slate-50 dark:bg-slate-800 rounded-full text-brand-600 shadow-inner group-hover:scale-110 transition-transform">
                                <ScanSearch className="h-20 w-20" />
                            </div>
                            <div className="text-center space-y-4">
                                <h3 className="text-xl font-black dark:text-white tracking-tighter uppercase">Structural Obligations</h3>
                                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed px-10">Upload a cover template to have AI respect safety zones, spines, and trim lines for Hardcover (Relié) formats.</p>
                            </div>
                         </div>
                    </div>
                  </div>
                ...
                <div className="animate-in fade-in slide-in-from-bottom-10 duration-700">
                   ...
                   <div className="flex flex-col lg:flex-row justify-between items-end gap-8 mb-14 bg-white dark:bg-slate-900 p-12 rounded-[3.5rem] border border-slate-100 dark:border-slate-800 shadow-2xl">
                       <div className="text-left">
                           ...
                           <div className="flex flex-wrap items-center gap-4">
                               <span className="bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-brand-100 dark:border-brand-800">
                                   {completedCount}/20 Pages
                               </span>
                               <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">
                                   {state.dimensions.width}" x {state.dimensions.height}"
                               </span>
                               <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">
                                   Age: {state.targetAge}
                               </span>
                           </div>
                       </div>
                       <div className="flex flex-wrap gap-4">
                           ...
                           <button 
                             onClick={handleGenerateAllInterior} 
                           ...
                           >
                             {isBatchGenerating ? <><StopCircle className="h-4 w-4" /> Stop Batch</> : <><PlayCircle className="h-4 w-4" /> Render All Interior</>}
                           </button>

                           <button onClick={() => generatePDF(state.metadata!, state.pages, state.dimensions, state.coverImage)} className="px-8 py-4 rounded-2xl bg-brand-600 text-white font-black text-[10px] uppercase tracking-widest flex items-center gap-3 shadow-xl hover:bg-brand-700 transition-all hover:-translate-y-1">
                             <Download className="h-4 w-4" /> Export Full KDP PDF
                           </button>
                       </div>
                   </div>

                   {/* Cover Management (Top priority as requested) */}
                   <section className="mb-14 bg-white dark:bg-slate-900 p-12 rounded-[4rem] border border-slate-100 dark:border-slate-800 shadow-2xl">
                       <div className="flex items-center justify-between mb-10 border-b dark:border-slate-800 pb-8">
                           <div>
                               <h2 className="text-3xl font-black flex items-center gap-3"><LayoutTemplate className="text-brand-600 h-8 w-8" /> Cover Studio</h2>
                               <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mt-1">Front & Back Cover Control (Hardcover Ready)</p>
                           </div>
                           <div className="flex gap-4">
                               <button onClick={() => coverUploadRef.current?.click()} className="px-6 py-3 rounded-xl border border-slate-200 dark:border-slate-700 font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-800">
                                 <Upload className="h-4 w-4" /> Upload Template
                               </button>
                               <input type="file" ref={coverUploadRef} className="hidden" accept="image/*,application/pdf" onChange={handleFileUpload} />
                           </div>
                       </div>
                       ...
                       <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
                           ...
                           <div className="space-y-6">
                               <div className="flex justify-between items-center">
                                   <span className="text-[11px] font-black uppercase text-slate-400 flex items-center gap-2"><Sparkles className="h-3 w-3 text-brand-500" /> Front Cover</span>
                                   <button onClick={() => generateCover('front')} disabled={loadingCover} className="bg-brand-600 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-brand-700 transition-all shadow-lg shadow-brand-500/20">
                                       {loadingCover ? <Loader2 className="animate-spin h-3 w-3" /> : <><Palette className="h-3 w-3" /> Render AI Front</>}
                                   </button>
                               </div>
                               <div className="aspect-[3/4] bg-slate-50 dark:bg-slate-950 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800 relative overflow-hidden flex items-center justify-center group shadow-inner">
                                   {state.coverImage ? (
                                       <>
                                           <img src={state.coverImage} className="w-full h-full object-cover transition-transform group-hover:scale-105 duration-700" />
                                           <div className="absolute inset-0 bg-brand-950/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                               <button onClick={() => saveAs(state.coverImage, 'FrontCover.png')} className="bg-white text-brand-950 p-4 rounded-2xl shadow-2xl hover:scale-110 transition-transform"><Download className="h-6 w-6" /></button>
                                           </div>
                                       </>
                                   ) : <div className="text-center opacity-20"><ImageIcon className="h-16 w-16 mx-auto mb-4" /><p className="font-black text-xs uppercase tracking-widest">Front Image Slot</p></div>}
                               </div>
                           </div>
                           {/* Back Cover Card */}
                           <div className="space-y-6">
                               <div className="flex justify-between items-center">
                                   <span className="text-[11px] font-black uppercase text-slate-400 flex items-center gap-2"><BookOpen className="h-3 w-3 text-slate-400" /> Back Cover</span>
                                   <button onClick={() => generateCover('back')} disabled={loadingBackCover} className="bg-slate-800 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-slate-900 transition-all shadow-lg">
                                       {loadingBackCover ? <Loader2 className="animate-spin h-3 w-3" /> : <><Palette className="h-3 w-3" /> Render AI Back</>}
                                   </button>
                               </div>
                               <div className="aspect-[3/4] bg-slate-50 dark:bg-slate-950 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800 relative overflow-hidden flex items-center justify-center group shadow-inner">
                                   {state.backCoverImage ? (
                                       <>
                                           <img src={state.backCoverImage} className="w-full h-full object-cover transition-transform group-hover:scale-105 duration-700" />
                                           <div className="absolute inset-0 bg-brand-950/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                               <button onClick={() => saveAs(state.backCoverImage, 'BackCover.png')} className="bg-white text-brand-950 p-4 rounded-2xl shadow-2xl hover:scale-110 transition-transform"><Download className="h-6 w-6" /></button>
                                           </div>
                                       </>
                                   ) : <div className="text-center opacity-20"><ImageIcon className="h-16 w-16 mx-auto mb-4" /><p className="font-black text-xs uppercase tracking-widest">Back Image Slot</p></div>}
                               </div>
                           </div>
                       </div>
                   </section>
                            ...
                            <div key={p.id} className={`bg-white dark:bg-slate-900 rounded-[3.5rem] border-2 shadow-2xl overflow-hidden transition-all flex flex-col group ${p.status === 'failed' ? 'border-red-500' : 'border-slate-100 dark:border-slate-800 hover:border-brand-500'}`}>
                                <div className="p-7 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                                    <div className="flex flex-col gap-1">
                                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Page {i + 1}</span>
                                        <span className="font-black text-[13px] tracking-tight dark:text-white truncate max-w-[180px]">{p.title}</span>
                                    </div>
                                    <div className={`w-3.5 h-3.5 rounded-full ${p.status === 'completed' ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.6)]' : (p.status === 'generating' ? 'bg-brand-500 animate-pulse' : 'bg-slate-200 dark:bg-slate-700')}`}></div>
                                </div>
                                
                                <div className="flex-1 aspect-[1/1.3] relative flex flex-col bg-white dark:bg-slate-950">
                                    {p.status === 'completed' && p.imageUrl ? (
                                        <div className="relative w-full h-full flex flex-col p-8">
                                            <div className="text-center mb-4">
                                                <p className="text-[10px] font-black uppercase text-slate-300 border-b border-slate-100 dark:border-slate-800 pb-1">{p.title}</p>
                                            </div>
                                            
                                            <img src={p.imageUrl} className="w-full flex-grow object-contain animate-in zoom-in-95 duration-1000" alt="Interior" />
                                            
                                            {p.saying && (
                                                <div className="mt-4 text-center">
                                                    <p className="text-[9px] font-bold italic text-slate-300 opacity-80 px-4">"{p.saying}"</p>
                                                </div>
                                            )}

                                            <div className="absolute inset-0 bg-brand-950/90 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-6 p-8 text-center">
                                                <div className="flex flex-col gap-2 items-center mb-2">
                                                    <Quote className="h-5 w-5 text-brand-400 mb-2" />
                                                    <p className="text-white text-[10px] font-bold italic leading-relaxed px-6">"{p.saying}"</p>
                                                </div>
                                                <div className="flex flex-wrap justify-center gap-3">
                                                    <button onClick={() => saveAs(p.imageUrl!, `Page_${i+1}.png`)} className="bg-white text-brand-950 px-5 py-3 rounded-2xl shadow-2xl hover:scale-110 transition-transform flex items-center gap-2 font-black text-[10px] uppercase tracking-widest"><ImageIcon className="h-4 w-4" /> PNG</button>
                                                    <button onClick={() => generateSingleImagePDF(p.imageUrl!, p.title, state.dimensions)} className="bg-white text-brand-950 px-5 py-3 rounded-2xl shadow-2xl hover:scale-110 transition-transform flex items-center gap-2 font-black text-[10px] uppercase tracking-widest"><FileText className="h-4 w-4" /> PDF</button>
                                                    <button onClick={() => setState(prev => ({...prev, pages: prev.pages.map(page => page.id === p.id ? {...page, status: 'pending', imageUrl: undefined} : page)}))} className="bg-red-500 text-white p-3 rounded-2xl shadow-2xl hover:scale-110 transition-transform"><Trash2 className="h-4 w-4" /></button>
                                                </div>
                                            </div>
                                        </div>
                                    ) : p.status === 'generating' ? (
                                        <div className="flex flex-col items-center justify-center flex-grow gap-6 p-10 text-center">
                                            ...
                                            <span className="text-[12px] font-black text-slate-400 uppercase tracking-[0.4em] animate-pulse">Rendering Design...</span>
                                        </div>
                                    ) : p.status === 'failed' ? (
                                        <div className="flex flex-col items-center justify-center flex-grow gap-6 text-center p-10">
                                            <AlertCircle className="h-16 w-16 text-red-500" />
                                            <p className="text-[11px] font-black text-red-500 uppercase tracking-widest">Resting...</p>
                                            <button onClick={() => generateSinglePage(p.id)} disabled={isBatchGenerating} className="bg-red-500 hover:bg-red-600 text-white px-10 py-5 rounded-[2rem] font-black text-[11px] uppercase tracking-widest shadow-2xl flex items-center gap-3 transition-transform hover:scale-105 disabled:opacity-50"><RotateCcw className="h-5 w-5" /> RE-RENDER</button>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center justify-center flex-grow gap-8 p-10 text-center">
                                            <div className="p-12 bg-slate-50 dark:bg-slate-900 rounded-full text-slate-200 dark:text-slate-800 shadow-inner">
                                                <Wand2 className="h-16 w-16" />
                                            </div>
                                            <div className="space-y-6">
                                                <button onClick={() => generateSinglePage(p.id)} disabled={isBatchGenerating} className="bg-brand-950 dark:bg-brand-600 hover:bg-brand-900 text-white px-12 py-6 rounded-[2.5rem] font-black text-[12px] uppercase tracking-[0.4em] shadow-3xl transition-all active:scale-95 flex items-center gap-4 disabled:opacity-50 mx-auto">
